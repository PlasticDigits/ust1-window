...
Tokens + oracle/window live on mainnet ([GitLab #19](https://gitlab.com/PlasticDigits/ust1-window/-/issues/19)). Window **11566** InstantWithdraw; oracle **11568** with Venus-normalized rate seeded **2026-08-08 11:04:37 UTC** (`R=1225104516022056627`). UST1 `add_minter(window)` and treasury `SetCw20Spender` done. Still pending: oracle-service Coolify (normalized ticks), live withdraw probe, window migrate for `fee_amount` / `fee_asset` wasm attrs ([#33](https://gitlab.com/PlasticDigits/ust1-window/-/issues/33)) — full registry in [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md). Redeem: [#20](https://gitlab.com/PlasticDigits/ust1-window/-/issues/20). Schema pin: [#21](https://gitlab.com/PlasticDigits/ust1-window/-/issues/21).
...
| **ust1-oracle** | `terra1fmht0t6svq3n24zx03nkfja0m40zhfyyxkdcvlrkl6u7gfe6aagq4gch8n` | **11568** | Operator `terra1hm3ph…`; Venus seed **2026-08-08 11:04:37 UTC** `R=1225104516022056627` (~1.225 FDUSD/vFDUSD) |
...
| `oracle-service` | Polls BSC `exchangeRateStored`, applies same policy as chain, broadcasts `UpdateRate` (including same-rate heartbeat after 4h), confirms DeliverTx wasm events or `State` fallback before liveness (poll/silence/heartbeat: [`skills/oracle-ops-poll-silence`](skills/oracle-ops-poll-silence/SKILL.md); confirm: [`skills/oracle-liveness-confirm`](skills/oracle-liveness-confirm/SKILL.md) ([#23](https://gitlab.com/PlasticDigits/ust1-window/-/issues/23), [#32](https://gitlab.com/PlasticDigits/ust1-window/-/issues/32))) |
...
- **INV-FEE-EVENT-001** — `ust1-common` `fee_amount_ust1` + `ust1-window` deposit/withdraw wasm `fee_amount` / `fee_asset` (raw UST1 withheld; never reconstruct from bps). Skill: [`skills/window-fee-amount-events`](skills/window-fee-amount-events/SKILL.md) ([#33](https://gitlab.com/PlasticDigits/ust1-window/-/issues/33); DEX [#614](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/614))
...
- **INV-ORACLE-LIVENESS-001** — oracle-service confirms DeliverTx wasm events (or `State` fallback) before silence/liveness success (not CheckTx alone); same-rate heartbeat after 4h throttle; `oracle-service/src/{confirm,terra_tx,liveness,main}.rs`, skills [`skills/oracle-liveness-confirm`](skills/oracle-liveness-confirm/SKILL.md), [`skills/oracle-ops-poll-silence`](skills/oracle-ops-poll-silence/SKILL.md) ([#23](https://gitlab.com/PlasticDigits/ust1-window/-/issues/23), [#32](https://gitlab.com/PlasticDigits/ust1-window/-/issues/32), audit C-3)
...
The `ust1-oracle-service` binary is intentionally **lightweight**: it uses **structured `tracing` logs only** (no Prometheus or in-process metrics server). Every `check_rate_update` / `oracle_heartbeat` and broadcast/confirm outcome is logged at `info` (policy / confirmed update) or `warn` (CheckTx / DeliverTx / event or state confirmation failure). **`BROADCAST_MODE_SYNC` CheckTx success alone does not count as success** (**INV-ORACLE-LIVENESS-001**): the service polls for DeliverTx inclusion and confirms wasm `update_rate` events (or retried oracle `State` when events are stripped) before recording liveness. Lagged LCD `State` after event proof does **not** fail-close. Same-rate Venus readings still submit `UpdateRate` once the 4h throttle allows so `last_update_sec` stays inside window `max_oracle_age_sec` (6h). If there has been **no confirmed on-chain oracle update** for longer than **`ORACLE_MAX_SILENCE_SECS`** (default **21600**, i.e. 6 hours — aligned with window `DEFAULT_MAX_ORACLE_AGE_SECS`), the process emits a **high-visibility `error!` liveness alert** on each poll tick. Default **`POLL_INTERVAL_SECS`** is **3600** (1h) so a missed tick does not consume the entire 6h staleness budget; on-chain throttle still skips broadcasts inside 4h (**INV-ORACLE-OPS-POLL-001** / **INV-ORACLE-OPS-SILENCE-001**, [glab #24](https://gitlab.com/PlasticDigits/ust1-window/-/issues/24), [glab #32](https://gitlab.com/PlasticDigits/ust1-window/-/issues/32)).
