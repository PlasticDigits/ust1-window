...
Ops/env: [`docs/DEPLOYMENT.md`](../../docs/DEPLOYMENT.md). Invariant index: [`README.md`](../../README.md).
...
| **INV-MINTER-001** | `UpdateMinter` clears old primary from `MINTERS` map (fork) | `cw20-mintable` |
| **INV-ORACLE-TICK-001** | Hung BSC/LCD cannot block past `TICK_TIMEOUT_SECS`; fail tick, not process | `oracle-service` `main` |
...
| `oracle-service/src/{main,bsc,terra_tx,config,liveness,healthz}.rs` | Timeouts, gas, SIGTERM, mutex recover, `/healthz` |
...
Key cases: dust deposit/withdraw unchanged balances; instantiate decimal inversion rejected; 6/6 happy path; account JSON missing sequence errors; gas `max(configured, network)`; `/healthz` → 200; poisoned liveness mutex recovers; tick timeout does not panic the process.
