...
Implements **only** audit C-2 recommendation #1 from [`audits/INTERNAL_KIMIK3_1786162831.md`](../../audits/INTERNAL_KIMIK3_1786162831.md). Issue: [ust1-window#22](https://gitlab.com/PlasticDigits/ust1-window/-/issues/22). Integration-suite coverage: [#28](https://gitlab.com/PlasticDigits/ust1-window/-/issues/28).

Cross-links: [docs/DEPLOYMENT.md](../../docs/DEPLOYMENT.md) [Emergency pause](../../docs/DEPLOYMENT.md#emergency-pause-oracle-circuit-breaker-vs-window), [TEST-16](../../docs/DEPLOYMENT.md#test-16--localterra-e2e-status-28), [README.md](../../README.md) invariants, companion skill [`window-instant-withdraw-cw20`](../window-instant-withdraw-cw20/SKILL.md) (guards must keep pause checks).
...
| `smartcontracts-terraclassic/tests/src/integration_tests.rs` | Same pause fail-closed path in integration suite (#28) |
| `oracle-service/src/main.rs` | Skip broadcast when `state.paused` (`TickAction::SkipPaused`; no liveness) |
| `docs/DEPLOYMENT.md` | Emergency pause runbook + TEST-16 status |
...
cargo test -p ust1-integration-tests --lib oracle_paused_blocks
```

Key cases: `state_surfaces_paused_flag_for_circuit_breaker`; `oracle_paused_blocks_deposit_and_withdraw_while_rate_fresh` (**both** window multitest and `ust1-integration-tests`); unpaused+stale → `OracleStale`; window pause vs oracle pause; unauthorized `SetPaused`.
